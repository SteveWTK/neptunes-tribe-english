...
import {
  getAdventureByThemeTag,
  getWorldByThemeTag,
} from "@/data/worldsConfig";

function AdventureHubContent() {
  const router = useRouter();
  const { user } = useAuth();
  const [currentAdventure, setCurrentAdventure] = useState(null);
  const [adventureWorld, setAdventureWorld] = useState(null);
  ...
  async function loadAdventureContent() {
    try {
      setLoading(true);
      const supabase = createClient();

      // Get current weekly adventure (theme in database)
      ...
      if (themeError || !themeData || themeData.length === 0) {
        console.error("Error loading adventure:", themeError);
        setLoading(false);
        return;
      }

      const currentThemeData = themeData[0]; // RPC returns an array
      setCurrentAdventure(currentThemeData);

      // Get the world this adventure belongs to
      const world = getWorldByThemeTag(currentThemeData.theme_tag);
      if (world) {
        setAdventureWorld(world);
      }
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-primary-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-6">
            Check back soon for this week&apos;s adventure!
          </p>
          <button
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-primary-900">
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            {adventureWorld && (
              <>
                <ChevronRight className="w-4 h-4 text-gray-400" />
                <button
                  onClick={() => router.push(`/worlds/${adventureWorld.slug}`)}
                  className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200"
                >
                  {adventureWorld.name}
                </button>
      ...
      {/* Hero Section */}
      <div className="bg-gradient-to-br from-primary-600 via-primary-700 to-accent-600 dark:from-primary-800 dark:via-primary-900 dark:to-accent-700 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="flex items-center gap-3 mb-4">
            <div className="p-3 bg-white/20 rounded-lg backdrop-blur-sm">
              {getEcosystemIcon(currentAdventure.ecosystem_type)}
            </div>
            <div>
              <div className="flex items-center gap-2 text-white/80 text-sm">
                <Calendar className="w-4 h-4" />
                <span>This Week&apos;s Adventure</span>
                {adventureWorld && (
                  <>
                    <span className="mx-2">·</span>
                    <span>{adventureWorld.name}</span>
                  </>
                )}
              </div>
              <h1 className="text-3xl font-bold">
                {currentAdventure.theme_title}
              </h1>
            </div>
          </div>
          ...
          <div className="mb-12">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
                ...
                {currentAdventure.theme_title} units. Begin with them to prepare
                for the practice activities in the lesson.
              </p>
            </div>
            ...
            <div>
              ...
              <p className="text-gray-700 dark:text-gray-300 mb-4">
                ...
                <span className="font-semibold">
                  {currentAdventure.theme_title} Explorer
                </span>{" "}
                badge!
              </p>
