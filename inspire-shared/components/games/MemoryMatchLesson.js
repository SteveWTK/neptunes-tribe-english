...
/**
 * MemoryMatchLesson - A memory match game component for vocabulary learning
 *
 * @param {Object} props
 * @param {Array} props.vocabulary - Array of vocabulary items with {id, en/english, pt/portuguese/translation, enImage?, ptImage?}
 * @param {Function} props.onComplete - Callback when game is completed with score
 * @param {string} props.lessonId - Unique lesson ID for localStorage state persistence
 * @param {string} props.vocabularySource - Optional source for fetching vocabulary ('personal', 'default')
 * @param {Function} props.onGameComplete - Alternative completion callback
 * @param {string} props.storageKey - Optional custom storage key for game state
 * @param {Object} props.translations - UI text translations
 * @param {string} props.saveToListEndpoint - API endpoint for saving words to practice list
 * @param {boolean} props.showSaveToList - Whether to show save to practice list feature (default: true)
 * @param {Object} props.emptyState - Empty state configuration
 */
export default function MemoryMatchLesson({
  vocabulary,
  onComplete,
  lessonId,
  ...
  translations = {},
  saveToListEndpoint = "/api/vocabulary/personal",
  showSaveToList = true,
  emptyState = {},
}) {
  ...
  const t = {
    ...
    matchEnglishPortuguese: "Match English words with their Portuguese translations",
    noVocabulary: "No vocabulary available for this lesson.",
    perfectMatch: "Perfect Match!",
    tryAgain: "Try Again!",
    congratulations: "Congratulations!",
    completedIn: "You matched all pairs in {attempts} attempts!",
    playAgain: "Play Again",
    attempts: "Attempts",
    matches: "Matches",
    matchedPairs: "Matched Pairs",
    savedToList: "Saved to Practice List",
    saveForPractice: "Save for Practice",
    saving: "Saving...",
    cards12: "3x4 (12 cards)",
    cards16: "4x4 (16 cards)",
    ...translations,
  };

  // Create a unique storage key for this lesson's memory match game
  const storageKey = customStorageKey || `memoryMatch_${lessonId || "default"}`;
        ...
        if (matched.length + 1 === cards.length / 2) {
          setTimeout(() => {
            ...
            toast.success(t.congratulations, {
              description: t.completedIn.replace("{attempts}", attempts + 1),
              duration: 5000,
            });
  ...
  const handleSaveToPersonalList = async (pair) => {
    ...
    try {
      ...
      const response = await fetch(saveToListEndpoint, {
        method: "POST",
    ...
    return (
      <div className="text-center p-8">
        <p className="text-gray-600 dark:text-gray-400">
          {emptyState.message || t.noVocabulary}
        </p>
      </div>
    );
  }
      ...
      {showSuccess && (
        <div className="fixed top-1/3 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 animate-bounce">
          <div className="bg-accent-500 bg-opacity-50 text-white px-6 py-3 rounded-full shadow-lg flex items-center gap-2">
            ...
            <span className="font-bold">{t.perfectMatch}</span>
          </div>
        </div>
      )}
      ...
      {showError && (
        <div className="fixed top-1/3 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 animate-pulse transition delay-150 duration-300 ease-in-out">
          <div className="bg-[#dc2626] bg-opacity-50 text-white px-6 py-3 rounded-full shadow-lg flex items-center gap-2">
            <X className="w-6 h-6" />
            <span className="font-bold">{t.tryAgain}</span>
          </div>
        </div>
      )}

      <div className="flex flex-col lg:flex-row lg:items-start justify-center gap-10 w-full max-w-7xl">
        {/* Game Grid Container - self-start prevents vertical stretching in flex row */}
      ...
      {isComplete ? (
        <div className="text-center space-y-4 animate-fadeIn">
          <div className="bg-gradient-to-r from-primary-500 to-accent-500 text-white p-8 rounded-xl shadow-xl">
            ...
            <h3 className="text-3xl font-bold mb-2">{t.congratulations}</h3>
            <p className="text-xl mb-4">
              {t.completedIn.replace("{attempts}", attempts)}
            </p>
