...
import WordSnakeOnboarding from "./WordSnakeOnboarding";
 ...
 *
 * @param {Array} clues - Array of word clues {clue, answer, hint, fact, targetImage}
 * @param {Function} onComplete - Callback when all words completed
 * @param {String} difficulty - "easy" or "hard" mode
 * @param {String} title - Game title (default: "Word Snake Challenge")
 * @param {String} storageKey - localStorage key for onboarding (default: "wordSnakeOnboarding")
...
const SPEED_DECREASE_PER_WORD = 15;

export default function WordSnakeLesson({
  clues = [],
  onComplete,
  difficulty = "easy",
  title = "Word Snake Challenge",
  storageKey = "wordSnakeOnboarding",
}) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const hasSeenOnboarding = localStorage.getItem(
        `${storageKey}_${difficulty}_completed`
      );
  ...
  useEffect(() => {
    ...
    snake.forEach((segment, i) => {
      ...
      if (i === 0) {
        ...
        const displayWordNoSpaces = collectedWord.replace(/\s+/g, "");
        const letterChar = displayWordNoSpaces[letterIndex] || "";
        ...
        if (letterChar) {
          ...
          ctx.textAlign = "center";
          ctx.textBaseline = "middle";
          ctx.fillText(letterChar, x + tileSize / 2, y + tileSize / 2);
        }
      }
    });
