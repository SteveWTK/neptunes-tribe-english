    ...
    >
      ...
      <div className="mt-4 flex flex-wrap justify-center align-middle gap-4 text-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-600 dark:text-gray-400">
            Marine Zones Completed ({oceanZones.length} loaded)
          </span>
        </div>
      </div>
      ...
      {/* {process.env.NODE_ENV === "development" && (
        <div className="mb-4 p-3 bg-gray-100 dark:bg-gray-800 rounded text-sm">
          ...
          <p>
            Highlighted regions: {highlightedRegions.length} (
            {highlightedRegions.join(", ")})
          </p>
          <p>
            Highlighted ocean zones: {highlightedOceanZones.length} (
            {highlightedOceanZones.join(", ")})
          </p>
          <p>
            Completed countries: {Object.keys(completedUnitsByCountry).length} (
            {Object.keys(completedUnitsByCountry).join(", ")})
          </p>
          <p>
            Completed ocean zones: {Object.keys(completedUnitsByOcean).length} (
            {Object.keys(completedUnitsByOcean).join(", ")})
          </p>
          {oceanZones.length > 0 && (
      ...
      {/* <div className="mt-2 space-x-2">
            <button
              onClick={() => {
                console.log("Test navigation to Brazil");
                router.push("/units?region=BR");
              }}
              className="bg-blue-500 text-white px-2 py-1 rounded text-xs"
            >
              Test: Navigate to Brazil
            </button>
            <button
              onClick={() => {
                console.log("Test navigation to Baltic Sea");
                router.push("/units?marine_zone=Baltic%20Sea");
              }}
              className="bg-green-500 text-white px-2 py-1 rounded text-xs"
            >
              Test: Navigate to Baltic Sea
            </button>
          </div>
        </div>
      )} */}
