...
// Updated fetchFeaturedUnits with premium filtering and sorting options
...
// Enhanced fetchSingleGapChallenges with premium filtering
export async function fetchSingleGapChallenges(
  ...
  // Premium filtering - if not premium user, only show non-premium challenges
  if (!isPremiumUser) {
    query = query.eq("is_premium", false);
  }
...
// Updated fetchFeaturedChallenges with premium filtering
export async function fetchFeaturedChallenges(isPremiumUser = false) {
  try {
    let query = supabase
      .from("single_gap_challenge")
